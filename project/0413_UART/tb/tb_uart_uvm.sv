    ...
    virtual function void build_phase(uvm_phase phase);
        ...
        if (!uvm_config_db#(virtual uart_if)::get(this, "", "u_if", u_if)) begin
            `uvm_fatal(get_type_name(),
                "can not find u_if in config db!!")
        end
    endfunction
    ...
    task run_phase(uvm_phase phase);
        ...
        forever begin
            ...
            `uvm_info(get_type_name(),
                $sformatf("Tx start: tx_data = 0x%02h", item.tx_data), UVM_HIGH)
            @(u_if.drv_cb);
            while(!u_if.drv_cb.tx_busy) @(u_if.drv_cb); // wait up to busy
            while(u_if.drv_cb.tx_busy) @(u_if.drv_cb); // busy
            `uvm_info(get_type_name(),
                $sformatf("Tx start: tx_data = 0x%02h", item.tx_data), UVM_HIGH)
...
module tb_uart_uvm ();
    ...
    uart_if u_if (.clk(clk), .reset(reset));
