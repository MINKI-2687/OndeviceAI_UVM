`ifndef RAM_SCOREBOARD_SV
    ...
    class ram_scoreboard extends uvm_scoreboard;
        ...
        virtual function void write(ram_seq_item item);
            ...
            if (item.wr) begin
                ...
                `uvm_info(get_type_name(),
                    $sformatf("Write Record: addr=0x%02h, data=0x%04h",
                ...
                if (item.rdata === expected) begin
                    pass_cnt++;
                    `uvm_info(get_type_name(),
                        $sformatf("PASS: addr=0x%02h, expected=0x%04h, real=0x%04h",
                            item.addr, expected, item.rdata), UVM_MEDIUM)
                end
                else begin
                    fail_cnt++;
                    `uvm_error(get_type_name(),
                        $sformatf("FAIL: addr=0x%02h, expected=0x%04h, real=0x%04h",
        ...
        virtual function void report_phase(uvm_phase phase);
            ...
            if (fail_cnt > 0) begin
                `uvm_error(get_type_name(),
                    $sformatf("TEST FAILED: %0d mismatches detected!", fail_cnt))
            end else begin
                `uvm_info(get_type_name(),
                    $sformatf("TEST PASSED: %0d all matches detected!", pass_cnt), UVM_LOW)
            end
        endfunction
    endclass
`endif
