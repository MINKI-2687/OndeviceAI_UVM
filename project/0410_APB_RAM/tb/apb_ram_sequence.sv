...
class apb_base_seq extends uvm_sequence #(apb_seq_item);
    ...
    task do_write(bit [7:0] addr, bit [31:0] data);
        ...
        if (!item.randomize() with {pwrite == 1'b1; paddr == addr; pwdata == data;})
            `uvm_fatal(get_type_name(), "do_write() Randomization Fail!")
        finish_item(item);
        `uvm_info(get_type_name(),
            $sformatf("do_write() SUCCESS!: addr=0x%02h, data=0x%08h", addr, data), UVM_MEDIUM)
        endtask
        ...
        task do_read(bit [7:0] addr, output bit [31:0] rdata);
            ...
            if (!item.randomize() with {pwrite == 1'b0; paddr == addr;})
                `uvm_fatal(get_type_name(), "do_read() Randomization Fail!")
            finish_item(item);
            rdata = item.prdata;
            `uvm_info(get_type_name(),
                $sformatf("do_read() SUCCESS!: addr=0x%02h, data=0x%08h", addr, rdata), UVM_MEDIUM)
            endtask
        endclass
