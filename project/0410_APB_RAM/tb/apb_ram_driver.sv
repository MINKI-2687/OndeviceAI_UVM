...
class apb_driver extends uvm_driver #(apb_seq_item);
  ...
  virtual function void build_phase(uvm_phase phase);
    ...
    if (!uvm_config_db#(virtual apb_if)::get(this, "", "vif", vif)) begin
      `uvm_fatal(get_type_name(), "[ERROR/DRV] can not find vif !!")
    end
  endfunction
  ...
  virtual task run_phase(uvm_phase phase);
    apb_bus_init();
    wait (vif.presetn == 1);
    `uvm_info(get_type_name(), "RESET -> 0. WAITING TRANSACTION...", UVM_MEDIUM)
  ...
  task drive_apb(apb_seq_item tx);
    ...
    `uvm_info(get_type_name(), $sformatf("DRV apb operation done: %s", tx.convert2string()),
              UVM_MEDIUM)
  endtask
endclass
`endif
